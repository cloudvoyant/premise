...
- **Safe workspace-root merging** — Merge direct registry files into the client workspace root while copying the selected template to `apps/<name>` or `libs/<name>`. Root conflicts use typed `mise.toml` rules, ordered registry-first Git policy, or complete-file choices. Premise preflights changed root tools and validates the complete two-level workspace before publication
